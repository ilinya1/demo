...
- **2026-09-09（操作日志 #65，按当前项目更新全部文档）** 用户要求「根据现在的项目更新所有文档」。基于代码逐项盘点（路由/接口/13 表/功能实现度/技术栈版本/测试），更新三份滞后文档：
  1. **`docs/功能模块说明.md`**：整篇重写（原为 9/4 的 MVP 规划版，含已废弃的 `check_out`/`hygiene_check`/`notice` 表、7 模块、把 Excel 导入导出当功能点），改为与当前实现一致：模块清单（鉴权/仪表盘/统计/基础数据/住宿/日常/系统设置/个人中心）、学生端 6 页、13 张表、测试/运行/demo 账号/端口、mock↔后端 切换，并把 **Excel 批量导入导出、调宿、通知、考勤、水电、违纪 明确为「二期未实现」**（避免误当作已有）。
  2. **`docs/项目交接文档.md`**：整体更新为「前端 Vue3 + 后端 Spring Boot + 13 表全栈已联调完成」现状版。补 13 表、t_college/sys_parameter、LONGTEXT base64 图片、bedNo 数字映射、apply/direct、JWT 拦截器、分页插件、9 类/30 @Test 独立测试库、演示数据与账号，删除过时表述、标注二期未实现。
  3. **`docs/开发设计文档.md`**：技术栈版本对齐实际（前端 Vite ^6.0.0/Vue ^3.5.0/Router ^4.5.0/Pinia ^3.0.0/Axios ^1.7.0/Element Plus ^2.14.0/补 ECharts ^6.1.0；后端 Starter 用 boot4+jsqlparser；鉴权为 JWT 拦截器非完整 Security）；13 表实体映射补齐 repair_type/sys_parameter/t_college；补充分页拦截器、bedNo 归一化、学院软关联、退宿双路径、独立测试库、LONGTEXT 图片；导入导出/独立上传接口标「预留未实现」；新增 v1.2 修订记录。
